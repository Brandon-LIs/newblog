...
const TITLE = '跃迁通道';
const DESCRIPTION = '通过跃迁通道随机穿梭到我的一个友链站点';
...
export default function Wormhole(): JSX.Element {
  ...
  const start = () => {
    ...
    setLog([]);
    setPhase('scanning');
    pushLog('>>> 正在启动跃迁引擎', 350);
    pushLog('>>> 校准能量矩阵…', 1100);
    pushLog(`>>> 已扫描 ${friends.length} 个友链节点`, 1700);
    pushLog(`>>> 锁定目标 → ${f.title}`, 2350);
    ...
    <Layout title={TITLE} description={DESCRIPTION} wrapperClassName="teleport-page">
      <main className={styles.root}>
        ...
        <div className={styles.content}>
          {phase === 'idle' && (
            <>
              <h1 className={styles.title}>跃迁通道</h1>
              <p className={styles.sub}>能量已就绪，向深空发起一次友链跃迁</p>
          ...
          {phase === 'scanning' && (
            <div className={styles.term}>
              <div className={styles.termBar}>
                ...
                <span className={styles.termTitle}>wormhole-drive --target=&lt;friend&gt;</span>
              </div>
